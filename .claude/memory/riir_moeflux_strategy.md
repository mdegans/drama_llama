...
| riir after 5d-8 | 7.38 | 7.25 | Apollo essay × 3 same-prompt sequential @ 512 max_tokens, M2 Max, --seed 42; cold = 1st on fresh server, warm = avg of runs 2+3 on same Session (7.26 / 7.25, very low variance). **Warm flat-to-slightly-down vs 5d-7** (7.25 vs 7.36, within noise; net ≈ -1.5%). Two consecutive perf-targeted slices (5d-7, 5d-8) have now landed flat — the framing on "host roundtrip elimination + GPU SDPA fast-path" being load-bearing was wrong. Diff suite stays bit-exact (40/40 green; eval_token cosine 1.0000000 rel 8.917e-7; eval_prompt cosine 1.0000000 rel 1.431e-6) and architectural alignment with C is now structural — non-last layers chain CMD3 → next-layer normed exactly as `infer.m:5677..5750` does. The chain is load-bearing for any future reorganization (e.g. eliminating the CMD1 cmdbuf entirely once per-head Q/K rms_norm + RoPE move to GPU). Next perf lever: **GPU per-head Q/K rms_norm + RoPE fusion** — the actual critical-path CPU work, now visible because the obvious nearby levers came back flat. |
| riir after 5d-9 | 7.69 | 8.15 | A3B Apollo essay × 3 sequential @ 512 max_tokens, M2 Max, --seed 42 (commit `914db09`). **Warm +12.4% vs 5d-8** (8.15 vs 7.25); warm runs 2+3 = 8.11 / 8.19, cold = 7.69 (up from 7.38). CPU ~200% / GPU ~60% (up from ~50%) — the depth-2 pipeline is engaging. A3B at 94% of C (warm 8.70). **A17B**: cold 1.95 / warm avg 1.96 (runs 2+3 = 1.96 / 1.96), against C path's 1.60 = **+22.5% over C**, CPU ~250% / GPU ~40-50%. Bigger model benefits more — longer GPU layers mean the cvwait we dropped was a larger fraction of wall-time. Diff suite 40/40 green in 619s (serial); eval tests bit-for-bit identical to post-5d-8 (cosine 1.0000000 rel 8.917e-7 / 1.431e-6). Two-set parity-keyed `data_prefetch` + depth-2 `DeferredRing` replacing `Option<DeferredState>`; per-layer wait dropped, drain only when ring full. The post-5d-8 profile's 76% main-thread cvwait should now be substantially diminished — re-profile pending. |
